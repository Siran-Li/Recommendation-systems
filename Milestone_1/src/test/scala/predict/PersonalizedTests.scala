...
class PersonalizedTests extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Test uniform unary similarities") {
    ...
    assert(within(baselinePredictor(train2)(1, 1), 4.046819980619529, 0.0001))

    // MAE
    assert(
      within(MAE(test2, baselinePredictor(train2)), 0.7604467914538644, 0.0001)
    )
  }
  ...
  test("Test ajusted cosine similarity") {
    ...
    // Compute personalized prediction for user 1 on item 1
    assert(
      within(personalizedPredictor(train2)(1, 1), 4.08702725876936, 0.0001)
    )

    // MAE
    assert(
      within(
        MAE(test2, personalizedPredictor(train2)),
        0.7372314455896454,
        0.0001
      )
    )
  ...
  test("Test jaccard similarity") {
    ...
    // Similarity between user 1 and user 2
    assert(
      within(computeJmSimilarities(train2)(0)(1), 0.03187250996015936, 0.0001)
    )

    // Compute personalized prediction for user 1 on item 1
    assert(
      within(personalizedJmPredictor(train2)(1, 1), 4.098236648361599, 0.0001)
    )

    // MAE
    assert(
      within(
        MAE(test2, personalizedJmPredictor(train2)),
        0.7556138848602905,
        0.0001
      )
    )
