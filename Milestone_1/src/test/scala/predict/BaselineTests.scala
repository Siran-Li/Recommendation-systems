...
class BaselineTests extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Compute global average") {
    assert(within(computeGlobalAverage(train2), 3.5264625, 0.0001))
  }
  test("Compute user 1 average") {
    assert(within(computeAvgRatingPerUser(train2)(1), 3.63302752293578, 0.0001))
  }
  test("Compute item 1 average") {
    assert(
      within(computeAvgRatingPerItem(train2)(1), 3.888268156424581, 0.0001)
    )
  }
  test("Compute item 1 average deviation") {
    assert(
      within(
        computeGlobalDevPerItem(
          train2,
          computeAvgRatingPerUser(train2)
        )(1),
        0.3027072341444875,
        0.0001
      )
  ...
  test(
    ...
    assert(
      within(MAE(test2, globalAveragePredictor(train2)), 0.94891098, 0.0001)
    )
    assert(
      within(MAE(test2, avgRatingUsersPredictor(train2)), 0.83834014, 0.0001)
    )
    assert(
      within(MAE(test2, avgRatingItemsPredictor(train2)), 0.82069514, 0.0001)
    )
    assert(within(MAE(test2, baselinePredictor(train2)), 0.76044679, 0.0001))
  }
}
