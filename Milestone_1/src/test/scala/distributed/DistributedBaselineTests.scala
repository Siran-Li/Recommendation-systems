...
class DistributedBaselineTests extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Compute global average") {
    assert(within(computeGlobalAvgRDD(train2), 3.5264625, 0.0001))
  }
  test("Compute user 1 average") {
    assert(
      within(
        computeAvgRatingPerUsersMapRDD(train2)
          .getOrElse(1, computeGlobalAvgRDD(train2)),
        3.63302752293578,
        0.0001
      )
    )
  ...
  test("Compute item 1 average") {
    assert(
      within(
        computeAvgRatingPerItemMapRDD(train2)
          .getOrElse(1, computeGlobalAvgRDD(train2))
          .toDouble,
        3.888268156424581,
        0.0001
  ...
  test("Compute baseline prediction for user 1 on item 1") {
    assert(
      within(baselinePredictorRDD(train2)(1, 1), 4.046819980619529, 0.0001)
    )
  }
  ...
  test(
    ...
    assert(
      within(
        maeRDD(test2, baselinePredictorRDD(train2)),
        0.7604467914538644,
        0.0001
      )
    )
