...
object Recommender extends App {
  ...
  val personalData = data.union(personal)
  val k = 300
  val predictor = knnPredictor(personalData, k)
  val predictionUser1 = predictor(1, 1)

  val user = 944
  val N = 3
  val top3 =
    getTopNRecommendationsForUser(personalData, movieNames, user, N, predictor)
  ...
  conf.json.toOption match {
    ...
    case Some(jsonFile) => {
      val answers = ujson.Obj(
        ...
        "R.1" -> ujson.Obj(
          "PredUser1Item1" -> predictionUser1 // Prediction for user 1 of item 1
        ),
        ...
        "R.2" -> top3.map(x => ujson.Arr(x._1, x._2, x._3))
      )
