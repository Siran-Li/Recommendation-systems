...
object Approximate {
  def main(args: Array[String]) {
    ...
    val measurements = (1 to scala.math.max(1,conf.num_measurements()))
      .map(_ => timingInMs( () => {
        MAEParallel(sc, test, distributedPredictor(sc, train, conf_k, partitionedUsers))
    ...
    val knnSimilarities = distributedSimilarities(
      ...
      computeNormalizedDeviation(
        train, 
        computeAvgRatingPerUser(train, train.mapActiveValues(x => if (x != 0.0) 1.0 else 0.0))
      ),
      conf_k,
      partitionedUsers
    )
    ...
    conf.json.toOption match {
      ...
      case Some(jsonFile) => {
        val answers = ujson.Obj(
          ...
          "AK.1" -> ujson.Obj(
            "knn_u1v1" -> knnSimilarities(0,0),
            "knn_u1v864" -> knnSimilarities(0,863),
            "knn_u1v344" -> knnSimilarities(0,343),
            "knn_u1v16" -> knnSimilarities(0,15),
            "knn_u1v334" -> knnSimilarities(0,333),
            "knn_u1v2" -> knnSimilarities(0,1)
          ),
          "AK.2" -> ujson.Obj(
            "mae" -> ujson.Num(mae) 
          ),
