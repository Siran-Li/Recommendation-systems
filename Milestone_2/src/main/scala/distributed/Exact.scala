...
object Exact {
  def main(args: Array[String]) {
    ...
    val PredUser1Item1 = knnPredictorParallel(sc, train, conf_k)(1, 1)
    val PredUser327Item2 = knnPredictorParallel(sc, train, conf_k)(327, 2)

    val measurements = (1 to conf.num_measurements()).map(x => timingInMs(() => {
        MAEParallel(sc, test, knnPredictorParallel(sc, train, conf_k))
    }))
